...
public class ServiceReplacerTest {
    ...
    public void testReplacesFailedMember() throws Exception {
        ...
        e1.emit(HASensors.ENTITY_FAILED, new HASensors.FailureDescriptor(e1, "simulate failure"));
    ...
    public void testAbandonsReplacementAfterNumFailures() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            entity.emit(HASensors.ENTITY_FAILED, new HASensors.FailureDescriptor(entity, "simulate failure"));
            if (i <= 3) {
                Asserts.succeedsEventually(new Runnable() {
                    @Override public void run() {
    ...
    private void assertEventuallyHasEntityReplacementFailedEvent(final Entity expectedCluster) {
        Asserts.succeedsEventually(new Runnable() {
            @Override public void run() {
                ...
                Assert.assertEquals(((HASensors.FailureDescriptor) Iterables.getOnlyElement(events).getValue()).getComponent(), expectedCluster, "events=" + events);
            }});
    }
}
