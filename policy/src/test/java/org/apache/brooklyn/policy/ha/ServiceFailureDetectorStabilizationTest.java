...
public class ServiceFailureDetectorStabilizationTest {
    ...
    public void setUp() throws Exception {
        events = new CopyOnWriteArrayList<SensorEvent<HASensors.FailureDescriptor>>();
        ...
        app.getManagementContext().getSubscriptionManager().subscribe(
                ...
                new SensorEventListener<HASensors.FailureDescriptor>() {
                    @Override public void onEvent(SensorEvent<HASensors.FailureDescriptor> event) {
                        events.add(event);
                    }
                });
        app.getManagementContext().getSubscriptionManager().subscribe(
    ...
    private void assertHasEvent(Sensor<?> sensor, Predicate<Object> componentPredicate, Predicate<? super CharSequence> descriptionPredicate) {
        for (SensorEvent<HASensors.FailureDescriptor> event : events) {
